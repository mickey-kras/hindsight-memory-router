...
[[ "$mode" == "real" ]] && compose_file="tests/integration/docker-compose.real.yml"
...
begin_check() { current_check="$1"; checks_total=$((checks_total + 1)); printf 'check %02d - %s ... ' "$checks_total" "$current_check"; }
pass_check() { checks_passed=$((checks_passed + 1)); echo "ok"; current_check=""; }
fail_check() { echo "failed" >&2; echo "${mode} integration smoke failed at check ${checks_total}: ${current_check}: $1" >&2; exit 1; }
run_check() { local name="$1"; shift; begin_check "$name"; "$@"; pass_check; }
...
docker compose -p "$project" -f "$compose_file" exec -T memory-router python -c 'import os,sys; sys.exit(1 if os.getenv("QUARANTINE_PRIVATE_KEY") else 0)' || fail_check "router runtime received QUARANTINE_PRIVATE_KEY"
pass_check

begin_check "router and internal Hindsight become reachable"
for _ in {1..60}; do
  if curl -fsS "${router_url}/health" >/dev/null 2>&1 && docker compose -p "$project" -f "$compose_file" exec -T memory-router python -c "import urllib.request; urllib.request.urlopen('http://hindsight:8888/health', timeout=3)" >/dev/null 2>&1; then break; fi
  sleep 2
done
curl -fsS "${router_url}/health" >/dev/null
docker compose -p "$project" -f "$compose_file" exec -T memory-router python -c "import urllib.request; urllib.request.urlopen('http://hindsight:8888/health', timeout=3)" >/dev/null
pass_check
...
if curl --max-time 2 -fsS "http://127.0.0.1:8888/health" >/dev/null 2>&1; then fail_check "internal Hindsight service is exposed on host port 8888"; fi
...
post_router() { curl -fsS -H "Authorization: Bearer ${router_token}" -H "Content-Type: application/json" -X POST "${router_url}$1" -d "$2"; }
admin_get() { curl -fsS -H "Authorization: Bearer ${admin_token}" "${router_url}$1"; }
admin_post() { local body="${2-'{}'}"; curl -fsS -H "Authorization: Bearer ${admin_token}" -H "Content-Type: application/json" -X POST "${router_url}$1" -d "$body"; }
retry_post_router() { local output; for _ in {1..60}; do if output="$(post_router "$1" "$2" 2>/dev/null)"; then printf '%s' "$output"; return 0; fi; sleep 2; done; post_router "$1" "$2"; }

decrypt_response() {
  local file="$1"
  printf '%s' "$QUARANTINE_PRIVATE_KEY" | docker run --rm -i -v "${file}:/input.json:ro" hindsight-memory-router:ci python -m memory_router.cli.decrypt_quarantine /input.json
}
...
printf '%s' "$stats" | python3 -c 'import json,sys; d=json.load(sys.stdin); assert d["pending_items"] >= 1; assert d["encrypted_bytes"] > 0'
if [[ "$mode" == "fake" ]] && grep -a "$unknown_marker" "${tmp_dir}/quarantine/quarantine.db" >/dev/null 2>&1; then fail_check "unknown plaintext leaked into SQLite quarantine database"; fi
pass_check
...
printf '%s' "$queue_response" | grep -q "$unknown_marker" && fail_check "admin queue leaked plaintext"
read_response="$(admin_get "/admin/quarantine/items/${unknown_id}")"
printf '%s' "$read_response" | grep -q "$unknown_marker" && fail_check "admin item leaked plaintext"
pass_check
...
unknown_plaintext="$(decrypt_response "$encrypted_file")"
...
admin_get "/admin/quarantine/queue" | grep -q "$unknown_id" && fail_check "rejected unknown item remained pending"
pass_check
...
approved_plaintext="$(decrypt_response "$encrypted_file")"
...
tamper_plaintext="$(decrypt_response "$encrypted_file")"
tamper_body="$(printf '%s' "$tamper_plaintext" | python3 -c 'import json,sys; v=json.load(sys.stdin); v["payload"]["body"]["items"][0]["content"]="changed"; print(json.dumps({"decrypted":v}, separators=(",",":")))')"
...
if [[ "$mode" == "fake" ]]; then
  ...
  recall_item="$(admin_get "/admin/quarantine/queue" | python3 -c 'import json,sys; items=json.load(sys.stdin)["items"]; print(next(i["quarantine_id"] for i in items if i["kind"] == "recalled_memory"))')"
...
retained=[e["bank_id"] for e in events if e.get("kind")=="retain"]
recalled=[e["bank_id"] for e in events if e.get("kind")=="recall"]
assert "main" in retained
assert "quarantine" not in retained
assert "research" not in recalled
assert "quarantine" not in recalled
PY
  pass_check
fi
