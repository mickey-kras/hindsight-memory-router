...
Confusable alternatives are capped at 32 per scanned field/window; exhaustion fails closed with `confusable_variant_limit`. One confusable-heavy field can therefore suppress a whole shared recall/facade response. NFKC first composes ordinary combining sequences. Remaining mark runs between ASCII-like word/separator boundaries and Default-Ignorable characters block as `invisible_unicode`; keycaps, Indic vowel signs, Arabic harakat, and Hebrew niqqud are preserved.
...
- `memory_router/confusables_ascii.json` is generated from Unicode UTS #39 17.0.0. Update the generated map and `THIRD_PARTY_NOTICES.md` together.
