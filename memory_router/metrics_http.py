...
async def metrics_endpoint_response(
    request: Request,
    *,
    ...
    if not admin_authorized(authorization, "read", admin_tokens):
        if resolver is not None and principal_token_present(authorization):
            return await _principal_metrics_response(
                request, resolver, auditor, repository, principal_rate, auth_failure_rate
            )
        if not await admin_auth(request, "read"):
            return JSONResponse(_AUTHENTICATION_REQUIRED, status_code=401)
    await admin_rate("GET")
    return await _metrics_response(repository)


async def _principal_metrics_response(
    request: Request,
    ...
    principal = await authenticate_principal(
        ...
        auditor=auditor,
        route_class="metrics",
        on_failure=lambda: auth_failure_rate("admin"),
    )
    ...
    await principal_rate(principal, SCOPE_QUARANTINE_REVIEW, "metrics")
    return await _metrics_response(repository)
