...
class HindsightLimits:
    ...
    def assert_retain_bounds(self, body: dict[str, Any]) -> None:
        if len(body["items"]) > self.config.max_retain_items:
            raise HttpError(413, "retain_item_limit_exceeded", "retain request contains too many memory items")
        if _string_value_bytes(body) > self.config.max_retain_content_bytes:
            raise HttpError(413, "retain_content_too_large", "retain content exceeds the configured byte limit")

    def assert_recall_bounds(self, body: dict[str, Any]) -> None:
        if len(body["query"].encode()) > self.config.max_recall_query_bytes:
            raise HttpError(413, "recall_query_too_large", "recall query exceeds the configured byte limit")
        if body.get("max_tokens") is not None and body["max_tokens"] > self.config.max_recall_max_tokens:
            raise HttpError(413, "recall_max_tokens_exceeded", "recall max_tokens exceeds the configured limit")
    ...
    async def _consume(self, kind: str, writer_id: str, writer_max: int, global_max: int) -> None:
        try:
            await self.limiter.consume_many([
                (f"hindsight:{kind}:writer:{writer_id}", writer_max, self.config.rate_limit_window_ms),
            ...
            raise HttpError(429, "hindsight_rate_limited", f"too many Hindsight {kind} requests", {"retry-after": str(retry_after)}) from exc
