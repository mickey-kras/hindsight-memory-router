...
from .review_repository import claim_review, finish_approve_retain, finish_reject_memory, interrupt_review, mark_memory_reviewed, postpone, remove
...
class QuarantineAdminService:
    def __init__(self, repository: Any, hindsight: Any, registry: Any, max_postpones: int = 3) -> None:
    ...
    async def read_item(self, quarantine_id: str) -> dict[str, Any]:
        ...
        if item.get("encrypted") is None:
            raise HttpError(409, "quarantine_payload_unavailable", "quarantine payload is no longer available")
        record = {key:value for key,value in item.items() if key != "encrypted" and key != "encrypted_bytes"}
    ...
    async def approve(self, quarantine_id: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if item["kind"] == "retain_request":
            ...
            if not isinstance(payload, dict) or payload.get("action") != "retain":
                raise HttpError(409, "invalid_quarantine_payload", "retain approval requires a retain request")
            ...
            if writer is None:
                raise HttpError(409, "writer_not_registered", "register the writer before approving its original retain request")
            ...
            try:
                ...
                raise
            await finish_approve_retain(self.repository, quarantine_id, at, {"writer_id":writer_id,"target_bank":writer.write_bank})
            return {"approved":True,"quarantine_id":quarantine_id,"target_bank":writer.write_bank}
        if item["kind"] == "recalled_memory":
            payload = decrypted["payload"]
            if not isinstance(payload, dict) or payload.get("action") != "recalled_memory" or not isinstance(payload.get("result"), dict):
                raise HttpError(409, "invalid_quarantine_payload", "recalled memory approval requires a recalled memory payload")
            bank_id = payload.get("bank_id")
            result = payload["result"]
            if bank_id != item.get("source_bank") or result.get("id") != item.get("source_memory_id"):
                raise HttpError(409, "quarantine_source_mismatch", "recalled memory source does not match quarantine metadata")
            await mark_memory_reviewed(self.repository, quarantine_id, "reviewed_allowed", iso_now())
            return {"reviewed":True,"allowed":True,"quarantine_id":quarantine_id,"source_bank":bank_id,"source_memory_id":result["id"]}
        raise HttpError(409, "invalid_review_action", "this quarantine item cannot be approved into memory")
    ...
    async def reject(self, quarantine_id: str) -> dict[str, Any]:
        ...
        if item["kind"] == "recalled_memory":
            ...
            if not bank_id or not memory_id:
                raise HttpError(409, "quarantine_source_missing", "recalled memory source metadata is missing")
            at = iso_now()
            claimed = await claim_review(self.repository, quarantine_id, "recalled_memory", at)
            try:
                await self.hindsight.invalidate_memory(bank_id, memory_id, f"Rejected by memory-router quarantine review {quarantine_id}")
            except Exception as exc:
                await interrupt_review(self.repository, claimed, at, exc)
                raise
            await finish_reject_memory(self.repository, quarantine_id, at)
            return {"reviewed":True,"allowed":False,"quarantine_id":quarantine_id,"source_bank":bank_id,"source_memory_id":memory_id}
        await remove(self.repository, quarantine_id, "rejected", iso_now())
        return {"rejected":True,"quarantine_id":quarantine_id}

    async def postpone(self, quarantine_id: str) -> dict[str, Any]:
        item = await self._require_reviewable(quarantine_id)
        if int(item.get("postpone_count") or 0) >= self.max_postpones:
            raise HttpError(409, "postpone_limit_reached", "maximum postpone count reached; approve, reject, or wait for QUARANTINE_ITEM_TTL_DAYS expiry")
        next_item = await postpone(self.repository, quarantine_id, iso_now())
        return {"postponed":True,"quarantine_id":quarantine_id,"count":next_item["postpone_count"]}

    async def stats(self) -> dict[str, int]:
        stats = await self.repository.stats(iso_now())
        return {key:stats[key] for key in ("total_items","pending_items","postponed_items","reviewed_allowed_items","reviewed_blocked_items","encrypted_bytes","event_count")}
    ...
    async def cleanup(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if older_than is not None:
            try:
                ...
                raise HttpError(400, "invalid_cleanup_time", "older_than must be an ISO timestamp") from exc
        ...
        if not isinstance(expected, int) or isinstance(expected, bool) or expected < 0:
            raise HttpError(400, "expected_count_required", "expected_count from a dry run is required")
    ...
    def _verify_exact(item: dict[str, Any], value: Any) -> dict[str, Any]:
        ...
        if not hmac.compare_digest(actual, str(item["sha256"])):
            raise HttpError(409, "quarantine_hash_mismatch", "decrypted quarantine content differs from the original item")
        for field in ("quarantine_id","created_at","reason","writer_id","source"):
            if decrypted.get(field) != item.get(field):
                raise HttpError(409, "quarantine_metadata_mismatch", "decrypted quarantine metadata differs from the stored item")
        return decrypted
