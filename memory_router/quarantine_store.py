...
class QuarantineStore:
    def __init__(self, public_key: str, repository: QuarantineRepository, limits: QuarantineLimits, rate_limiter: Any) -> None:
        decode_public_key(public_key)
        self.public_key = public_key
        self.repository = repository
        self.limits = limits
        self.rate_limiter = rate_limiter
        self.capacity = Capacity(limits.max_pending_items, _effective_writer_limit(limits), limits.max_encrypted_bytes)
    ...
    async def put(self, input_: dict[str, Any]) -> dict[str, str]:
        ...
        if encrypted_bytes > self.limits.max_item_bytes:
            raise HttpError(413, "quarantine_item_too_large", "encrypted quarantine item exceeds configured size limit")

        async def operation(session: Any) -> dict[str, str]:
            existing = await self.repository.get(quarantine_id)
            if input_["kind"] in {"retain_request","recall_request"} and input_.get("dedupeKey") and existing and existing["status"] not in {"pending","postponed"}:
                raise HttpError(409, "quarantine_request_in_review", "matching quarantine request is already being reviewed")
            ...
            mode = "memory" if input_["kind"] == "recalled_memory" else "request" if input_["kind"] in {"retain_request","recall_request"} and item.get("dedupe_key") else "id"
    ...
    def _encrypt(self, input_: dict[str, Any], quarantine_id: str) -> dict[str, Any]:
        decrypted: dict[str, Any] = {"quarantine_id":quarantine_id,"created_at":input_["timestamp"],"reason":input_["reason"],"payload":input_["payload"]}
    ...
    def _build_item(self, input_: dict[str, Any], quarantine_id: str, encrypted: dict[str, Any]) -> dict[str, Any]:
        item: dict[str, Any] = {
            "quarantine_id":quarantine_id,"created_at":input_["timestamp"],"updated_at":input_["timestamp"],
            "kind":input_["kind"],"reason":input_["reason"],"writer_id":input_.get("writerId"),"source":input_.get("source"),
            "source_bank":input_.get("sourceBank"),"source_memory_id":input_.get("sourceMemoryId"),
            "source_content_sha256":input_.get("sourceContentSha256"),"dedupe_key":input_.get("dedupeKey"),
            "sha256":encrypted["sha256"],"encrypted":encrypted,"status":"pending","postpone_count":0,"requarantine_count":0,
        }
        if self.limits.item_ttl_days > 0:
            try:
                created = datetime.fromisoformat(input_["timestamp"].replace("Z", "+00:00"))
            except ValueError as exc:
                raise HttpError(400, "invalid_quarantine_timestamp", "quarantine timestamp must be an ISO timestamp") from exc
            item["expires_at"] = (created + timedelta(days=self.limits.item_ttl_days)).astimezone(timezone.utc).isoformat(timespec="milliseconds").replace("+00:00", "Z")
        return item

    async def _known_identity(self, input_: dict[str, Any], item_exists: bool) -> bool:
        if input_.get("dedupeKey") and input_["kind"] in {"security_event","retain_request","recall_request"}:
            return item_exists
        if input_["kind"] == "recalled_memory" and input_.get("sourceBank") is not None and input_.get("sourceMemoryId") is not None:
            return await self.repository.find_memory_state(input_["sourceBank"], input_["sourceMemoryId"]) is not None
        return False
    ...
    async def _charge(self, input_: dict[str, Any], known: bool, session: Any) -> None:
        ...
        if known:
            key = "quarantine-requarantine-ops:auth-audit" if auth_audit else "quarantine-requarantine-ops"
        ...
        if self.limits.rate_limit_max <= 0 or await self._capacity_exhausted(input_["timestamp"]):
            return
        if auth_audit:
            await session.consume_many([("quarantine-writes:auth-audit", self.limits.rate_limit_global_max, window)])
            return
        writer = "unknown-writer" if input_["reason"] == "unknown_writer" else input_.get("writerId") or "unknown-writer"
        family = request_family_identity(input_["kind"], input_["reason"], input_.get("writerId"), input_["payload"])
        identities: list[tuple[str,str,int,int]] = []
        if family:
            identities.append((f"quarantine-request-family:{family[0]}", family[1], self.limits.distinct_family_limit_max, window))
    ...
    async def _capacity_exhausted(self, at: str) -> bool:
        ...
        return stats["pending_items"] + stats["postponed_items"] >= self.capacity.max_pending_items or stats["encrypted_bytes"] >= self.capacity.max_encrypted_bytes
...
def _effective_writer_limit(limits: QuarantineLimits) -> int:
    if limits.max_pending_items_per_writer <= 0:
        return 0
    return min(limits.max_pending_items_per_writer, max(0, limits.max_pending_items - 1), max(0, (limits.max_encrypted_bytes - 1) // limits.max_item_bytes))
