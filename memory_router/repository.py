...
async def insert_event(
    ...
    await tx.execute(
        ...
        (str(uuid.uuid4()), quarantine_id, at, event_type, json.dumps(details or {}, separators=(",", ":"))),
    )
...
class QuarantineRepository:
    ...
    async def _assert_capacity(self, tx: Tx, item: dict[str, Any], existing: dict[str, Any] | None, capacity: Capacity, at: str) -> None:
        ...
        existing_pending = bool(existing_live and existing_live.get("status") in {"pending", "postponed"})
        ...
        if capacity.max_pending_items_per_writer > 0:
            ...
            scoped_row = await tx.fetchone(
                "SELECT COUNT(*) count FROM quarantine_items WHERE status IN ('pending','postponed') AND NOT(expires_at IS NOT NULL AND expires_at<=?) AND " + scope_sql,
                (at, *scope_params),
            ) or {}
