...
class QuarantineRepository:
    ...
    async def store(self, item: dict[str, Any], capacity: Capacity, *, mode: str, at: str) -> None:
        async with self.db.transaction(capacity_lock=True) as tx:
            ...
            if existing:
                ...
                if (
                    mode == "memory"
                    and existing["status"] == "reviewed_allowed"
                    and existing.get("source_content_sha256") != item.get("source_content_sha256")
                ):
                    await self._reopen_changed_memory(tx, existing["quarantine_id"], item)
                    return
                await self._refresh(
                    tx,
    ...
    async def _reopen_changed_memory(
        self, tx: Tx, quarantine_id: str, item: dict[str, Any]
    ) -> None:
        envelope = json.dumps(item["encrypted"], separators=(",", ":"), ensure_ascii=False)
        await tx.execute(
