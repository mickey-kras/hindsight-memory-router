...
For a local real-Hindsight smoke run, set the same explicit image input first:

```sh
export HINDSIGHT_TEST_IMAGE='ghcr.io/vectorize-io/hindsight:VERSION@sha256:DIGEST'
bash tests/integration/smoke.sh real sqlite
```

Use the version/digest from the CI resolution or the prepared `release.json`.
The Compose file rejects an unset input.
