...
| SonarQube Community gate | Implemented; live validation pending | Analysis and quality gate run nightly with deduplicated issue reporting; release tags require a successful `main` publish run for the same commit |
...
SonarQube Community runs nightly against `main`: analysis, quality gate, and finding sync report through deduplicated issues instead of blocking publication. Release tags publish only commits that already completed the publish workflow successfully on `main`.
