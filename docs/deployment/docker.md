...
Compose builds one Python Memory Router image and uses it for:
...
The router mounts the public-key volume read-only and does not mount the private-key volume. The initializer is the only service that mounts the private-key volume, and it runs with `network_mode: none`. The image owns `/app/data` as the non-root `memoryrouter` user so SQLite and WAL files remain writable inside the named volume.
...
The long-running container runs as the non-root `memoryrouter` user.
